...
def _wa_post(payload: dict) -> bool:
    """Send to WhatsApp API; log any error; never raise."""
    try:
        url = f"{GRAPH_API_BASE}/messages"
        headers = {"Authorization": f"Bearer {WHATSAPP_TOKEN}"}
        r = requests.post(url, json=payload, headers=headers, timeout=15)
        if not r.ok:
            logging.error("WA POST failed: %s | Payload=%s", r.text, payload)
            return False
        return True
    except Exception as e:
        logging.exception("WA POST error: %s | Payload=%s", e, payload)
        return False
...
def send_listings_menu(to: str, category_key: str):
    ...
    for item in listings:
        short_title = clip(f"{item['id']} {cat_title}", 24)
        short_desc = clip(f"{item['title']} — {item['desc']}", 72)
        rows.append({
    ...
    body_text = clip(f"We have the following listings for *{cat_title}*. Select an option to see photos.", 1024)
    section_title = clip(f"{cat_title} Muither Villa", 24)
    ...
    ok = _wa_post(payload)
    if not ok:
        _send_listings_menu_text_fallback(to, category_key)
